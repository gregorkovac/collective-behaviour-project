...
class Simulation:
    ...
    def simulate(self, deltaTime, params):
        ...
        sp = self.get_spp_properties(self.pos, self.pos, self.dir)
    ...
    @staticmethod
    def get_spp_properties(pos0, pos1, dir):
    ...
    def flow_offset(self, sp, params, delta=1e-6):
        ...
        U = u.reshape((SP.num_fish, SP.num_fish, 2))
        ...
        sp_dx = self.get_spp_properties(pos_dx, self.pos, self.dir)
        u_dx = self.flow_u(sp_dx, params)
        u_dx = (u_dx - u) / delta

        pos_dy = np.column_stack((self.pos[:, 0], self.pos[:, 1] + delta))
        sp_dy = self.get_spp_properties(pos_dy, self.pos, self.dir)
        ...
        # this is should be better, but has not been tested enaugh
        u_grad_i = u_grad_ij.reshape((SP.num_fish, SP.num_fish, 2, 2))
    ...
    def resolve_collisions(self, params):
        ...
        if len(idx) > 0:
            ...
            x = np.bincount(idx[:, 0], weights=offset[:, 0], minlength=SP.num_fish)
            y = np.bincount(idx[:, 0], weights=offset[:, 1], minlength=SP.num_fish)
